...
def on_message(client, userdata, msg):
    ...
    if msg.topic == 'sensor' and is_json(out):
       ...
       # print(now)
        ...
        #print("temperature:"+out["temp"]+",humidity:"+out["humi"]+",co2:"+out["co2"]+",light:"+out["light"]+",time:"+s)
        dataInfo.objects.create(temperature=out["temp"],humidity=out["humi"],co2=out["co2"],light=out["light"],time=s)
        time.sleep(5)
    if msg.topic == 'warn' and is_json(out):
        out = json.loads(out)
        
        now = datetime.datetime.now()
       # print(now)
        s = now.strftime('%Y/%m/%d %H:%M:%S')
        print("co2:"+out["co2"]+",time:"+s)
        rfidInfo.objects.create(co2=out["co2"],time=s)
        #time.sleep(2)
